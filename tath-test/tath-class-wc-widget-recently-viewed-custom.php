 ...
 */
class WC_Widget_Recently_Viewed_TATH_CUSTOM extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_id          = 'tath_woocommerce_recently_viewed_products';
		$this->widget_name        = __( 'TATH Recent Viewed Products', 'woocommerce' );
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
	...
	public function widget( $args, $instance ) {
		...
		if ( $r->have_posts() ) {
			...
			echo wp_kses_post( apply_filters( 'woocommerce_before_widget_product_list', '<div class="px-0 pb-2 row product_list_widget">' ) );
			...
			while ( $r->have_posts() ) {
				$r->the_post();
				wc_get_template( 'content-widget-product-col2.php', $template_args );
			}
...
//add_action( 'widgets_init', create_function( '', 'return register_widget( "WC_Widget_Recently_Viewed_TATH_CUSTOM" );' ) );
