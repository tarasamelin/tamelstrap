...
function tath_get_the_ip_upload() {
...
// Add the shortcode
add_shortcode( 'contact-form-upload', 'tath_contact_form_upload_sc' );
function tath_contact_form_upload_sc( $atts ) {
	...
	if ( $_SERVER['REQUEST_METHOD'] == 'POST' ) {
		...
		if ( $error == false ) {
			...
			$email_message = "IP: " . tath_get_the_ip_upload(). "\n\n";
...
//    ------- upload function end
	$email_form = '<form class="contact-form" method="post" enctype="multipart/form-data" action="'.get_permalink().'">
		...
		<div class="form-group mb-3">
			<input class="pt-1 pb-1 btn btn-outline-primary" id="cf_send" type="submit" value="'.$label_submit.'" name="upload" />
		</div>
	</form>';
