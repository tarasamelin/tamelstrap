...
<form class="pb-3 mb-3 border border-left-0 border-right-0 border-top-0 cart grouped_form" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
		...
		<tbody>
			...
			foreach ( $grouped_products as $grouped_product_child ) {
				...
				foreach ( $grouped_product_columns as $column_id ) {
					...
					echo '<td class="align-middle woocommerce-grouped-product-list-item__' . esc_attr( $column_id ) . '">' . apply_filters( 'woocommerce_grouped_product_list_column_' . $column_id, $value, $grouped_product_child ) . '</td>'; // WPCS: XSS ok.
	...
	<?php if ( $quantites_required ) : ?>
		...
		<button type="submit" class="ml-4 mt-0 pt-1 pb-1 text-capitalize btn btn-outline-secondary rounded-0 single_add_to_cart_button button alt"><?php echo esc_html( $product->single_add_to_cart_text() ); ?></button>
